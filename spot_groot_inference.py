"""
Spot + GR00T N1 Visual Novelty Detection with Ridgeback Franka Pick-and-Place

Combined script:
- Spot robot walks forward, queries GR00T for visual novelty, stops on detection
- Ridgeback Franka mobile manipulator performs pick-hold-return-place in the same scene
- Both robots operate independently in the Simple Warehouse environment
"""
...
# ---------------------------------------------------------------------------
# ZMQ serialisation helpers (for GR00T policy server communication)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Ridgeback Franka mobile manipulator (pick-hold-return-place)
# ---------------------------------------------------------------------------
...
class RidgebackFrankaMobile:
    """Ridgeback Franka mobile manipulator: drive to cube, pick, return, place."""
    ...
    def setup_mobile_base(self, stage):
        ...
        print(f"[Franka] Found Franka robot at: {self._franka_prim_path}")
        ...
        print(f"[Franka] Ridgeback base at {self.start_position}, cube at x={self.cube_offset}m")
        return self._mobile_base_prim_path

    def _discover_scene_prims(self, stage):
        franka_path = self._franka_prim_path or "/World/Franka"
        skip_prefixes = (
            franka_path, self._mobile_base_prim_path,
            "/World/Warehouse", "/World/Spot", "/World/ObstacleCube",
        )
        ...
        for prim in stage.Traverse():
            ...
            try:
                ...
                print(f"[Franka] Cached scene prim {path} at {orig}")
        ...
        for path in self._scene_prim_originals:
            ...
            if prim.IsValid():
                ...
                if "cube" in name or "block" in name:
                    ...
                    print(f"[Franka] Graspable cube: {path}")
                    return
    ...
    def _find_gripper_prim(self, stage):
        ...
        for path in candidates:
            ...
            if prim.IsValid():
                ...
                print(f"[Franka] Gripper prim: {path}")
                return
    ...
    def reset(self):
        ...
        print(f"[Franka] Reset to start position {self.start_position}")
    ...
    def forward(self, ik_method: str):
        ...
        if self._state == MobileState.INIT:
            print("[Franka] INIT -> MOVE_TO_CUBE")
            ...
            if reached or self._state_step_count > 2000:
                print("[Franka] MOVE_TO_CUBE -> WAIT_SETTLED")
            ...
            if self._settled_steps > 30:
                print("[Franka] WAIT_SETTLED -> PICK_CUBE")
            ...
            if cube_lifted or pick_done or self._pick_steps > 800:
                ...
                print(f"[Franka] PICK_CUBE -> RETURN_TO_START ({reason} after {self._pick_steps} steps)")
            ...
            if reached or self._state_step_count > 2000:
                print("[Franka] RETURN_TO_START -> WAIT_SETTLED_RETURN")
            ...
            if self._settled_steps > 30:
                print("[Franka] WAIT_SETTLED_RETURN -> PLACE_CUBE")
            ...
            if cube_pos is not None and cube_pos[2] > 0.05:
                ...
                print("[Franka] PLACE_CUBE -> DONE")
                self._state = MobileState.DONE

        if self._step_count % 200 == 0:
            print(f"[Franka] Step {self._step_count}, State: {self._state.name}")
...
# ---------------------------------------------------------------------------
# Spot camera / movement constants
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Combined runner: Spot + GR00T + Ridgeback Franka
# ---------------------------------------------------------------------------
...
class SpotGR00TRunner(object):
    ...
    def _add_cube_obstacle(self):
        ...
        UsdGeom.Xformable(cube_prim).AddScaleOp().Set(Gf.Vec3d(0.15, 0.15, 0.15))
        ...
        try:
            ...
            if rgba is not None and rgba.shape[0] > 0:
                ...
                if not self._camera_ready and np.any(rgb > 0):
                    ...
                    print(f"[Camera] Camera producing valid frames (shape={rgb.shape})")
                return rgb
        except Exception as e:
    ...
    def on_physics_step(self, step_size) -> None:
        if self.first_step:
            ...
            print("[Franka] Initialized. Starting pick-hold-return-place task.")
            return
        ...
        if self.needs_reset:
            ...
            self.needs_reset = False
            self.first_step = True
            print("[Spot] Episode reset.")
            print("[Franka] Episode reset.")
            return
        ...
        if self._ridgeback_initialized and not self._ridgeback_franka.is_done():
            self._ridgeback_franka.forward(self._ik_method)

    def run(self) -> None:
        print("")
        print("=" * 60)
        print("  Spot + GR00T + Ridgeback Franka Combined Demo")
        print("=" * 60)
        print(f"  Spot task: {self._task_description}")
        print(f"  Spot forward speed: {self._forward_speed}")
        print(f"  GR00T query interval: {self._query_interval}s")
        print(f"  Warmup period: {self._warmup_seconds}s")
        print(f"  Franka cube offset: {self._ridgeback_franka.cube_offset}m")
        print(f"  Franka IK method: {self._ik_method}")
    ...
    def _sub_keyboard_event(self, event, *args, **kwargs) -> bool:
        if event.type == carb.input.KeyboardEventType.KEY_PRESS:
            if event.input.name == "SPACE":
                print("[Combined] Resetting episode...")
                self.needs_reset = True
...
def main():
    parser = argparse.ArgumentParser(
        description="Spot + GR00T + Ridgeback Franka Combined Demo"
    )
    parser.add_argument(
        "--task",
        type=str,
