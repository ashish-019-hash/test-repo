...
try:
    ...
    print("[WARNING] paho-mqtt not installed. MQTT camera/status publishing disabled.")
...
MQTT_PUBLISH_RATE_HZ = 2  # publish camera images via MQTT at ~2 Hz (lower than ROS2 to avoid flooding)
...
class H1GR00TRunner(object):
    def __init__(
        ...
        # --- MQTT publisher for n8n integration ---
        ...
        self._mqtt_publish_interval = 1.0 / MQTT_PUBLISH_RATE_HZ
        self._mqtt_h1_status_published = False  # track whether we already published "stopped"
        self._setup_mqtt()
    ...
    def _setup_mqtt(self):
        """Initialize MQTT client for publishing camera images and H1 status to n8n."""
        ...
        try:
            ...
            print(f"[MQTT] Command center topic: {MQTT_COMMAND_CENTER_TOPIC}")
        except Exception as e:
    ...
    def _publish_camera_mqtt(self):
        ...
        try:
            # Encode RGB image as JPEG, then base64 for MQTT transport
    ...
    def _publish_h1_status_mqtt(self, status, message):
        ...
        Args:
            status: Robot status string, e.g. "stopped" or "walking"
