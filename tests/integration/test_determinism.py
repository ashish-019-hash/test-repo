...
def test_two_runs_are_byte_identical(fmt: str, fixtures_dir: Path, tmp_path: Path, cfg: AppConfig) -> None:
    a, b = tmp_path / "a", tmp_path / "b"
    ra = run_fixture(fixtures_dir, fmt, a, cfg)
    rb = run_fixture(fixtures_dir, fmt, b, cfg)
...
def test_graph_export_is_deterministic(fixtures_dir: Path, tmp_path: Path, cfg: AppConfig) -> None:
    cfg = cfg.model_copy(update={"output": cfg.output.model_copy(update={"graph_export": "json"})})
    a, b = tmp_path / "a", tmp_path / "b"
...
def test_run_json_holds_the_volatile_fields(fixtures_dir: Path, tmp_path: Path, cfg: AppConfig) -> None:
    out = tmp_path / "out"
    result = run_fixture(fixtures_dir, "md", out, cfg)
