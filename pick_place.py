...
class RidgebackFrankaExperimental(FrankaExperimental):
    ...
    The Ridgeback Franka adds a mobile base to the Franka arm.
    The mobile base joints precede the arm joints in the DOF ordering.
    This subclass adjusts the joint indices so that IK and gripper
    commands target the correct Franka arm and gripper DOFs.
    """

    # Number of mobile base DOFs (x, y, yaw for the Ridgeback holonomic base)
    BASE_DOF_COUNT = 3
    ...
    def __init__(
        ...
        # Initialize the Articulation base class directly (skip FrankaExperimental's __init__
        # which would try to load the standard Franka USD)
        ...
        if create_robot:
            # Default state for the Ridgeback Franka:
            # [base_x, base_y, base_yaw, joint1..joint7, finger1, finger2]
        ...
        # Gripper positions (same as standard Franka)
    ...
    def set_end_effector_pose(self, position, orientation, ik_method="damped-least-squares"):
        """Set the end effector to a specific pose using IK on arm joints only.

        Overrides the parent to account for the base DOF offset so that IK
        only computes and applies deltas for the 7 arm joints (indices 3-9),
        not the 3 base joints (indices 0-2).
        """
        ...
        # Apply delta to arm joints only (indices BASE_DOF_COUNT to BASE_DOF_COUNT+7)
    ...
    def open_gripper(self):
        """Open the gripper (adjusted for base DOF offset)."""
        finger1_idx = self.BASE_DOF_COUNT + 7
        finger2_idx = self.BASE_DOF_COUNT + 8
        self.set_dof_position_targets(self.gripper_open_position, dof_indices=[finger1_idx, finger2_idx])

    def close_gripper(self):
        """Close the gripper (adjusted for base DOF offset)."""
        finger1_idx = self.BASE_DOF_COUNT + 7
        finger2_idx = self.BASE_DOF_COUNT + 8
        self.set_dof_position_targets(self.gripper_closed_position, dof_indices=[finger1_idx, finger2_idx])
